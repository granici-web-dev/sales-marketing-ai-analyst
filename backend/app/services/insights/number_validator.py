...
def cross_check(parsed: object, kpi_snapshot: dict, problems_input: list[dict]) -> bool:
    ...
    AI-06: Verifies that numeric values in the summary are grounded in the
    input KPI data and anomaly estimates (±2% tolerance).

    Algorithm:
      1. Build reference set: kpi_snapshot values (day-of-month from date field;
         percentage forms for conversion fields); problem current_value/expected_value
         (+ hours form for minute-scale values; percentage form for fractions).
      2. If reference set empty → return True (no numbers to check against).
      3. Extract numbers from parsed.summary only (not problem descriptions —
         descriptions legitimately contain domain-knowledge figures like offer
         validity periods that are not derived from the KPI snapshot).
      4. For each extracted number > 10 that is not a calendar year (1900–2100):
         check if within ±2% of ANY reference value.
      5. If a number is NOT within ±2% of any reference value → return False.
      6. Return True if all checked summary numbers match at least one reference value.
    ...
    for key, val in kpi_snapshot.items():
        if val is None:
            continue
        if key == "date":
            # Add day-of-month to reference so "27 mai 2026" in summary doesn't fail
            try:
                date_str = str(val)
                day = int(date_str.split("-")[2]) if "-" in date_str else getattr(val, "day", None)
                if day is not None:
                    reference_values.append(float(day))
    ...
    for problem in problems_input:
        for field_name in ("estimated_loss_ron", "current_value", "expected_value"):
            val = problem.get(field_name)
            if val is None:
                continue
            try:
                float_val = float(str(val))
                reference_values.append(float_val)
                # Values > 100 may be in minutes — also add hours form for narrative matching
                if float_val > 100:
                    reference_values.append(float_val / 60)
                # Fractional values (0–1) may be expressed as percentages in narrative
                if 0 < float_val < 1:
                    reference_values.append(float_val * 100)
            except (ValueError, TypeError):
                pass

        # Also add scalar numeric values from context_json (e.g. drop_pct, max_days_stuck)
        ctx = problem.get("context_json") or {}
        if isinstance(ctx, dict):
            for v in ctx.values():
                if isinstance(v, (int, float)):
                    try:
                        fv = float(v)
                        reference_values.append(fv)
                        if 0 < fv < 1:
                            reference_values.append(fv * 100)
                        if fv > 100:
                            reference_values.append(fv / 60)
                    except (ValueError, TypeError):
                        pass
    ...
    # Extract numbers from the summary only.
    # Problem descriptions contain legitimate domain-knowledge numbers (e.g., "30-day offer validity",
    # "25.000–40.000 RON per typical offer") that are not derived from the KPI snapshot —
    # checking them against input data would produce false positives.
    ...
    for number in narrative_numbers:
        if number <= 10:
            ...
            continue
        # Skip calendar years — these appear in date strings ("27 mai 2026") and are not KPI values
        if 1900 <= number <= 2100:
            continue
